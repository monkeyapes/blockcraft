...
for (const [tier, head] of TOOL_TIERS) {
  ITEM_ART[`pickaxe_${tier}`] = toolTile('pickaxe', head);
  ITEM_ART[`axe_${tier}`] = toolTile('axe', head);
  ITEM_ART[`shovel_${tier}`] = toolTile('shovel', head);
}
...
for (const [material, tint] of ARMOR_MATERIALS) {
  for (const slot of ['head', 'chest', 'legs', 'feet'] as const) {
    ITEM_ART[`armor_${slot}_${material}`] = armorTile(slot, tint);
  }
}
...
for (const [tier, head] of TOOL_TIERS) {
  ITEM_ART[`sword_${tier}`] = swordTile(head);
}
